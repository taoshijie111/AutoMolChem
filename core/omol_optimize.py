...
def read_conformer_info(conformer_dir: Path) -> Tuple[str, int, int]:
    ...
    info_file = conformer_dir / "conformer" / "info.txt"
...
def optimize_conformer_directory(conformer_dir: Path, calc, fmax: float, steps: int, 
    ...
    try:
        xyz_file = conformer_dir / "conformer" / "conformer.xyz"
...
def find_conformer_directories(base_dir: Path, skip_completed: bool = True) -> list[Path]:
    """Find all directories containing conformer/conformer.xyz files, including in batch subdirectories."""
    conformer_dirs = []
...
def main():
    ...
    print("Loading uma-s-1 model...")
    predictor = pretrained_mlip.get_predict_unit("uma-s-1", device=args.device, inference_settings=InferenceSettings(tf32=False, 
